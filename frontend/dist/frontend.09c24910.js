...
async function cargarRecursos() {
    try {
        ...
        (${r.categoria || "Sin categor\xeda"}) - 
        <em>${r.descripcion || "Sin descripci\xf3n"}</em>
        <button class="btn-eliminar" data-id="${r._id}">\u{274C}</button>
      `;
        ...
        document.querySelectorAll(".btn-eliminar").forEach((btn)=>{
            btn.addEventListener("click", async (e)=>{
                ...
                if (confirm("\xbfSeguro que quieres eliminar este recurso?")) {
...
if (uploadForm) uploadForm.addEventListener("submit", async (e)=>{
    ...
    if (!titulo) {
        alert("Por favor, ingresa un t\xedtulo.");
        return;
    }
    try {
        const res = await fetch(`${API_URL}/recursos`, {
        ...
        if (res.ok) {
            alert("\u2705 \xa1Recurso agregado con \xe9xito!");
            uploadForm.reset();
            cargarRecursos();
        } else alert("\u274C Error al enviar la gu\xeda.");
    } catch (err) {
...
//# sourceMappingURL=frontend.09c24910.js.map
